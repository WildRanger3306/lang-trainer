...
Цель — список уровня спецшколы (УМК Верещагина / Афанасьева, 2–11 класс) к концу основной школы: **143 глагола, A1–B2**, британский вариант (UK). Файл: [`docs/words/irregular/en.json`](words/irregular/en.json) — A1 52, A2 22, B1 28, B2 41.
...
Формы — британские. Где в BrE норма оба варианта (`learnt/learned`, `dreamt/dreamed`, `burnt/burned`, `spelt/spelled`, `spoilt/spoiled`), хранятся оба, `-t` первым. Формы только NAmE не берём: `got` (не `gotten`), `knelt`, `spat`, `spun`, `forbade`, `slid`. `sew` — `sewed`, `sewn/sewed`. `hang` — только `hung` (`hanged` «казнить» — правильный). `bear` — `borne/born`.
...
- `pattern` — тип образования по написанию первого варианта: `AAA` (cut–cut–cut) 19, `ABB` (buy–bought–bought) 67, `ABA` (come–came–come) 3, `AAB` (beat–beat–beaten) 1, `ABC` (go–went–gone) 53. Идёт в темы `irr_*` для фильтра. `read` по написанию `AAA`, по звучанию `ABB`.
...
- Вручную также пропуски `en_UK` (`leant`, `upset` и др.) и артефакты словаря: `dreamt` [drˈɛmpt] → [drˈɛmt], `arise/arose` [ɐ…] → [ər…], `hear` [hˈiə] → [hˈɪə]. `ate` — британское [ɛt].
...
- Метку `Irregular verbs` несут все 143 глагола из `en.json`; 62 глагола Starlight получают её при слиянии, рядом со своим учебником. Учебники в фильтре — через ИЛИ, так что `Starlight 7` + `Irregular verbs` даёт формы и для `buy` из Starlight.
- `forms` заменяет `native_to_foreign`, потому что «читать → read, read, read» целиком покрывает «читать → read». Нагрузка на 143 глагола: 286 новых карточек вместо 429.
...
- Оборот на узком экране: строкой или столбиком для длинных форм (`understand` — `understood`) — решаем при тестировании вёрстки: начинаем со строки, проверяем на ширине 380px.
...
- спрягалку для FR — отдельная схема позже.
