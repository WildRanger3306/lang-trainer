...
| `introduced_on` | день первого показа (для лимита новых) |

Сессия: все due по фильтру + до 15 новых в день на язык.

## Ответ (`card_review`)

Одна строка на каждое нажатие «помню» / «не помню». Нужна для перформанса за 7/30 дней (снимок `card_progress` этого не даёт).
